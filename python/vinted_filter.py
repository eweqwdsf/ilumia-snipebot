...
class FilterEngine:
    def __init__(self, start_ts: float, test_mode: bool = False):
    ...
    def evaluate_item(self, item: dict) -> tuple[bool, str]:
        ...
        # 1. Preis Check
        if not (FILTER_MIN_PRICE <= price <= FILTER_MAX_PRICE):
        ...
        has_hype_brand = any(c in brand or c in title for c in CORE_BRANDS)
        has_hype_keyword = any(k in title for k in HYPE_KEYWORDS)
